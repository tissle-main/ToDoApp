...
public static class CategoryDbSeeder
{
    ...
    {
        public async ValueTask<List<CategoryEntity>> SeedDatabase(
        ...
        public async ValueTask<Dictionary<Guid, List<CategoryEntity>>> SeedDatabaseForAllUsers(
            AppDbContext dbContext,
            CancellationToken cancellationToken,
        ...
        )
        {
            ...
            {
                List<CategoryEntity> list = await thisFaker.Clone().WithUserId(userId).SeedDatabase(dbContext, cancellationToken, min, max);
