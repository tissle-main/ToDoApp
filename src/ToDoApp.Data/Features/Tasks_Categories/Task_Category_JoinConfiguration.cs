...
public sealed class Task_Category_JoinConfiguration : IEntityTypeConfiguration<Task_Category_JoinEntity>
{
    ...
    public void Configure(EntityTypeBuilder<Task_Category_JoinEntity> builder)
    {
        ...
        builder.HasOne(e => e.Task).WithMany(t => t.Categories).HasForeignKey(e => e.TaskId).OnDelete(DeleteBehavior.Cascade);
        builder.HasOne(e => e.Category).WithMany(c => c.Tasks).HasForeignKey(e => e.CategoryId).OnDelete(DeleteBehavior.Cascade);
    }
    #endregion
}